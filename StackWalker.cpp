...
private:
  ...
  BOOL GetModuleListPSAPI(HANDLE hProcess)
  {
    ...
    hMods = (HMODULE*) mymalloc(sizeof(HMODULE) * (TTBUFLEN / sizeof HMODULE));
    tt = (char*) mymalloc(sizeof(char) * TTBUFLEN);
    tt2 = (char*) mymalloc(sizeof(char) * TTBUFLEN);
...
//    }
    ...
    void *pData = mymalloc(4096); // reserve enough memory, so the bug in v6.3.5.1 does not lead to memory-overwrites...
    if (pData == NULL)
    {
...
BOOL StackWalker::LoadModules()
{
  ...
  if ( (this->m_options & SymBuildPath) != 0)
  {
    ...
    szSymPath = (char*) mymalloc(nSymPathLen);
    if (szSymPath == NULL)
    {
      SetLastError(ERROR_NOT_ENOUGH_MEMORY);
      return FALSE;
...
#error "Platform not supported!"
  ...
  pSym = (IMAGEHLP_SYMBOL64 *) mymalloc(sizeof(IMAGEHLP_SYMBOL64) + STACKWALK_MAX_NAMELEN);
  ...
  for (frameNum = 0; ; ++frameNum )
  {
    ...
    if (s.AddrPC.Offset != 0)
    {
      ...
      if (this->m_sw->pSGSFA(this->m_hProcess, s.AddrPC.Offset, &(csEntry.offsetFromSmybol), pSym) != FALSE)
      {
        ...
        strcpy_s(csEntry.name, /*psu>>*/sizeof( csEntry.name ),/*<<psu*/ pSym->Name);
        // UnDecorateSymbolName()
